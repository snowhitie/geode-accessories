...
public:
    ...
    void rebuildFolderList() {
        ...
        // Large, high-contrast Android-friendly ADD button. Keep it inside the folder column
        // and on the same line as the header so it is always visible and easy to tap.
        auto addFolderBtn = makeTextOnlyButton("+ ADD", ccColor3B{ 55, 185, 95 }, 0.36f,
            this, menu_selector(AccessoriesPopup::onAddFolder), 108.f, 32.f);
        if (addFolderBtn) {
            addFolderBtn->setTag(1090);
            addFolderBtn->setAnchorPoint({ 0.5f, 0.5f });
            addFolderBtn->setPosition({ kFolderColX + 92.f, kListTopY + 10.f });
            m_menu->addChild(addFolderBtn, 20);
        }

        if (folders.empty()) {
            auto hint = CCLabelBMFont::create("(empty - tap + ADD\nto import an\naccessory folder)", "bigFont.fnt");
            hint->setScale(0.28f);
