...
public:
    ...
    std::string accessoriesRoot() {
        // Prefer the user-facing folder next to the current GD executable.
        // If GDPS Switcher changes the game directory and that folder is no
        // longer present, fall back to the mod save directory. This keeps the
        // accessory library available across GDPS/game-directory switches.
        auto gdRoot = dirs::getGameDir().string() + PATH_SEP + "Accessories";
        if (winfs::dirExists(gdRoot) || winfs::dirExists(gdRoot + PATH_SEP))
            return gdRoot;

        auto stableRoot = (Mod::get()->getSaveDir() / "Accessories").string();
        return stableRoot;
    }
    ...
    void scanAccessories() {
        m_folders.clear();
        ensureRootExists();
        std::string root = accessoriesRoot();

        auto topLevelFolders = winfs::listSubdirs(root);
        if (topLevelFolders.empty()) {
            log::warn("Accessories: no folders found in {} - drop an accessory folder in there "
                      "(e.g. Accessories/crown/) and reopen the Accessories window.", root);
        }
        ...
        for (auto const& folderName : topLevelFolders) {
            ...
            if (partSubdirs.empty()) {
                // no part-subfolders: the accessory folder's own PNGs are one implicit part.
            ...
            if (folder.parts.empty()) {
                log::warn("Accessories: folder '{}' ({}) has no usable parts - put PNGs directly "
                          "inside it, or in subfolders of it, named/ending with p1/p2/line/glow/"
                          "selfcolor.png.", folder.name, folder.fullPath);
            } else {
                log::info("Accessories: folder '{}' -> {} part(s)", folder.name, folder.parts.size());
            }
        ...
        int totalParts = 0;
        for (auto& f : m_folders) totalParts += (int)f.parts.size();
        log::info("Accessories: scan complete - {} folder(s), {} usable part(s) total.", m_folders.size(), totalParts);
    }
